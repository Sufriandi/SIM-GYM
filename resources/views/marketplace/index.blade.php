            ...
            @else
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 lg:gap-8">
                    @foreach($products as $p)
                        ...
                        {{-- PRODUCT CARD PREMIUM (HIGH CONTRAST & ELEGANT) --}}
                        <div class="group relative rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col
                            ...
                            <a href="{{ route('guest.marketplace.show', $slug) }}" class="relative w-full aspect-[3/4] bg-[#12141a] block overflow-hidden border-b border-white/[0.08]">
                                ...
                                <div class="absolute top-3 right-3 z-20">
                                    ...
                                    @elseif($stok <= 3)
                                        <span class="px-2.5 py-1 bg-amber-950/90 backdrop-blur-md text-amber-300 text-[9px] font-bold uppercase tracking-wider rounded-full border border-amber-500/40 shadow-md flex items-center gap-1.5">
                                            <span class="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span> Sisa {{ $stok }}
                                        </span>
                                    @endif
                                </div>
                            ...
                            {{-- Card Content --}}
                            <div class="p-4 sm:p-5 flex flex-col flex-grow bg-[#15171e]/95 relative z-10">

                                {{-- Title --}}
                                <a href="{{ route('guest.marketplace.show', $slug) }}" class="block mb-2.5">
                                    <h3 class="text-sm sm:text-base font-bold text-white leading-snug line-clamp-2 h-[2.75rem] group-hover:text-gold-400 transition-colors" title="{{ $p->nama }}">
                                        {{ $p->nama }}
                                    </h3>
                                </a>
                                ...
                                <div class="mb-3">
                                    @if($ready)
                                        <span class="inline-flex items-center gap-1.5 text-[10px] font-semibold text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 ring-2 ring-emerald-400/20 animate-pulse"></span> Stok: {{ $stok }} unit
                                        </span>
                                    @else
                                ...
                                <div class="mt-auto pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                                    <div>
                                        ...
                                        <span class="text-base sm:text-lg font-display font-extrabold text-white group-hover:text-gold-300 transition-colors">
                                            Rp {{ number_format((int)$p->harga, 0, ',', '.') }}
                                        </span>
                                    </div>

                                    {{-- Action Gold Button --}}
                                    <a href="{{ route('guest.marketplace.show', $slug) }}"
