...
<header
    ...
    x-data="{
        ...
        initNotifications() {
            ...
            // IMPORTANT: polling tetap jalan
            this.pollTimer = setInterval(() => {
                this.fetchNotif(false);
            }, this.pollMs);

            // saat balik tab, refresh list dropdown (tanpa bunyi)
    ...
    initSound();
    initNotifications();" @click.away="showNotifications = false; showProfile = false"
    role="banner">
    <div class="w-full px-4 lg:px-8 flex items-center justify-between gap-3 md:gap-4
               flex-wrap">
        {{-- KIRI: tombol mobile + breadcrumb --}}
        <div class="flex items-center gap-3 min-w-0 flex-1">
            <button @click="window.dispatchEvent(new CustomEvent('toggle-mobile-menu'))"
            ...
            <div class="min-w-0">
                <nav class="flex items-center text-xs sm:text-sm font-bold text-text-muted" aria-label="Breadcrumb">
                    ...
                    @if ($pageTitle)
                        <i data-lucide="chevron-right" class="w-4 h-4 mx-1.5 text-text-muted flex-shrink-0"
                            stroke-width="3"></i>
        ...
        <div class="flex items-center gap-2 lg:gap-3 flex-shrink-0">
            <div
                ...
                <input type="text" x-model="searchQuery" placeholder="Cari member, produk…"
            ...
            {{-- Notifikasi --}}
            <div class="relative">
                <button @click.stop="openNotifications()"
                    ...
                    :class="{ 'ring-2 ring-gold-500/30': showNotifications }" aria-label="Notifications"
                    ...
                    <span id="adminNotifBadge"
                        ...
                        x-show="unreadCount > 0" x-text="unreadCount > 9 ? '9+' : String(unreadCount)"></span>
                </button>

                {{-- Dropdown notifikasi --}}
                <div x-show="showNotifications" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    <div
                        class="px-4 py-3 border-b border-brand-borderSoft flex items-center justify-between bg-brand-shell/80">
                        <div class="flex items-center gap-3">
                            <h3 class="text-sm font-semibold text-text-main">Notifikasi</h3>

                            <button type="button" @click.stop="toggleSound()"
                                ...
                                x-text="soundEnabled ? 'Suara: ON' : 'Suara: OFF'" aria-label="Toggle sound"></button>
                        </div>

                        <span class="text-xs text-accent-400 font-medium" x-show="unreadCount > 0"
                                        ...
                                        <div class="flex gap-3">
                                            <div class="mt-1.5 flex-shrink-0">
                                                <span class="inline-flex h-2 w-2 rounded-full"
                                                    :class="n.is_read ? 'bg-brand-borderSoft' : 'bg-accent-500'"></span>
                                            </div>

                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm text-text-main font-medium mb-0.5 truncate"
                                                    x-text="n.title"></p>
                                                <p class="text-xs text-text-muted truncate" x-text="n.body"></p>
                                                <span class="text-[10px] text-text-muted mt-1 inline-block"
                                                    x-text="formatTime(n.created_at)"></span>
                                            </div>
                                        </div>
                                    </a>
                                </template>
            ...
            {{-- Profil --}}
            <div class="relative">
                <button @click.stop="showProfile = !showProfile; showNotifications = false"
                    ...
                    :class="{ 'ring-2 ring-gold-500/30': showProfile }" aria-label="User menu"
                    ...
                    <i data-lucide="chevron-down" class="w-4 h-4 text-text-muted transition-transform duration-200"
                        :class="{ 'rotate-180': showProfile }"></i>
                </button>

                <div x-show="showProfile" x-cloak x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 scale-95 -translate-y-2"
                    x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                    x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0"
