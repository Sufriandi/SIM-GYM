...
}" @toggle-member-sidebar.window="mobileOpen = !mobileOpen" class="relative z-40"
    ...
    <aside
        ...
        <div
            ...
            <button type="button"
                ...
                @click="mobileOpen = false" aria-label="Tutup menu">
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-6 custom-scrollbar">
            ...
            <div class="space-y-2">
                ...
                {{-- Dashboard --}}
                <a href="{{ route('member.dashboard') }}" @click="mobileOpen = false"
                    ...
                    <i data-lucide="layout-dashboard"
                        class="w-5 h-5 transition-transform duration-300
                               {{ $isDashboard ? 'text-gold-300' : 'group-hover:scale-110' }}"></i>
                    <span>Dashboard</span>
            ...
            <div class="space-y-2">
                ...
                <button type="button" @click="openKehadiran = !openKehadiran"
                    ...
                    :aria-expanded="openKehadiran.toString()" aria-controls="kehadiran-submenu">
                    <i data-lucide="check-square"
                        class="w-5 h-5 transition-transform duration-300
                               {{ $kehadiranGroupActive ? 'text-gold-300' : 'group-hover:scale-110' }}"></i>
                ...
                {{-- SUBMENU --}}
                <div id="kehadiran-submenu" x-show="openKehadiran" x-cloak x-transition class="space-y-1 mt-1"
                    role="menu">
                    {{-- Izin Latihan --}}
                    <a href="{{ route('member.izin_latihan.index') }}" @click="mobileOpen = false"
                        ...
                        role="menuitem" aria-current="{{ $isIzin ? 'page' : 'false' }}">
                        <i data-lucide="file-text"
                            class="w-4 h-4 {{ $isIzin ? 'text-gold-300' : 'text-brand-silver/70' }}"></i>
                        <span>Izin Latihan</span>
                    </a>

                    {{-- Riwayat Kehadiran --}}
                    <a href="{{ route('member.kehadiran.index') }}" @click="mobileOpen = false"
                        class="group flex items-center gap-3 pl-12 pr-4 py-2.5 text-sm transition-all duration-200 rounded-xl
                               {{ $isKehadiran
                                   ? 'text-gold-300 font-medium bg-gradient-to-r from-gold-500/10 to-transparent'
                                   : 'text-brand-silver hover:text-white hover:bg-brand-gunmetal/30' }}"
                        role="menuitem" aria-current="{{ $isKehadiran ? 'page' : 'false' }}">
                        <i data-lucide="qr-code"
                            class="w-4 h-4 {{ $isKehadiran ? 'text-gold-300' : 'text-brand-silver/70' }}"></i>
            ...
            <div class="space-y-2">
                ...
                {{-- Produk Gym --}}
                <a href="{{ route('member.produk_gym.index') }}" @click="mobileOpen = false"
                    class="group flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-medium
                           transition-all duration-300
                           {{ $isProduk
                               ? 'bg-gradient-to-r from-gold-500/20 to-transparent text-gold-300 shadow-lg shadow-gold-500/20'
