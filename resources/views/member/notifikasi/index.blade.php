{{-- resources/views/member/notifikasi/index.blade.php --}}
...
<x-layouts.member
    ...
    page-subtitle="Semua notifikasi yang masuk untuk member."
>
    <div x-data="{ search: @js($search ?? ''), filter: @js($filter ?? 'all') }">
        ...
        <div class="mt-6 mb-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="relative w-full max-w-md">
                <form action="{{ route('member.notifikasi.index') }}" method="GET" id="searchForm">
                    <input type="hidden" name="filter" :value="filter">
                    <div class="flex items-center w-full rounded-full border border-brand-borderSoft bg-brand-card
            ...
            <div class="flex items-center gap-2 flex-wrap justify-end">
                ...
                {{-- Filter --}}
                <form action="{{ route('member.notifikasi.index') }}" method="GET">
                ...
                {{-- Mark all read --}}
                <form action="{{ route('member.notifikasi.read_all') }}" method="POST">
                    @csrf
                    <button
                        type="submit"
                ...
                {{-- Hapus semua --}}
                <form id="delete-all-notif" action="{{ route('member.notifikasi.hide_all') }}" method="POST" class="inline-block">
                    @csrf
                    <button
                        type="button"
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    <a
                                        href="{{ route('member.notifikasi.go', $n->id) }}"
                                ...
                                <div class="mt-3 flex items-center gap-2 flex-wrap">
                                    @if(!$isRead)
                                        <form action="{{ route('member.notifikasi.read_one', $n->id) }}" method="POST">
                                            @csrf
                                            <button
                                                type="submit"
                                    ...
                                    <form
                                        ...
                                        action="{{ route('member.notifikasi.hide_one', $n->id) }}"
                                        method="POST"
                                        class="inline-block"
                                    >
                                        @csrf
