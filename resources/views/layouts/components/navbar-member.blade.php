...
<nav class="fixed top-0 left-0 right-0 h-16 bg-dark-card border-b-2 border-gold-900 z-50">
    <div class="container mx-auto px-4 lg:px-8 flex items-center justify-between h-full">
        ...
        <div class="flex items-center space-x-6">
            ...
            <a href="{{ route('member.dashboard') }}" class="flex items-center space-x-2 flex-shrink-0">
                <img src="{{ asset('images/logo.png') }}" alt="BETA GYM Logo" class="h-10 w-auto" style="filter: drop-shadow(0 0 5px rgba(200, 168, 112, 0.5));">
                <span class="text-lg font-heading tracking-wider text-gold"><span class="text-accent">BETA</span> GYM</span>
            </a>

            {{-- Navigasi Link (Desktop) --}}
            <div class="hidden lg:flex items-center space-x-6">
                <a href="{{ route('member.dashboard') }}" class="text-sm font-semibold uppercase {{ Request::is('member/dashboard') ? 'text-gold' : 'text-text-secondary' }} hover:text-gold transition duration-200">Dashboard</a>

                {{-- ✅ PERBAIKAN: Hanya satu link untuk Izin Latihan --}}
                <a href="{{ route('member.izin_latihan.index') }}" class="text-sm font-semibold uppercase {{ Request::is('member/izin-latihan*') ? 'text-gold' : 'text-text-secondary' }} hover:text-gold transition duration-200">Izin Latihan</a>
            </div>
        </div>
            ...
            @auth
                ...
                <div class="flex items-center">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center space-x-2 lg:space-x-3 px-2 py-2 rounded-gym bg-dark-surface border border-gold-900 hover:border-gold transition duration-200 focus:outline-none group">
                                <div class="relative">
    ...
    <div id="mobile-menu" class="hidden lg:hidden bg-dark-card border-t-2 border-gold-900 shadow-lg absolute top-16 left-0 w-full">
        <div class="flex flex-col space-y-1 p-4">
            <a href="{{ route('member.dashboard') }}" class="px-4 py-2 rounded-md {{ Request::is('member/dashboard') ? 'bg-dark-surface text-gold' : 'text-text-secondary' }} hover:bg-dark-surface hover:text-gold transition duration-200 font-semibold uppercase">Dashboard</a>

            <a href="{{ route('member.izin_latihan.index') }}" class="px-4 py-2 rounded-md {{ Request::is('member/izin-latihan*') ? 'bg-dark-surface text-gold' : 'text-text-secondary' }} hover:bg-dark-surface hover:text-gold transition duration-200 font-semibold uppercase">Izin Latihan</a>
        </div>
    </div>
</nav>
