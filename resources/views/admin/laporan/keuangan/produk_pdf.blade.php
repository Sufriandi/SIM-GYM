{{-- resources/views/admin/laporan/keuangan/produk_pdf.blade.php --}}
@php
    use Carbon\Carbon;
    ...
    $sumTopQty = $topProduk ? $topProduk->sum('qty') : 0;
@endphp
<!doctype html>
<html lang="id">
...
<body>
    @include('admin.laporan.partials.pdf_header', [
        'reportTitle' => 'Laporan Penjualan Produk Retail',
    ...
    <table class="kpi-table" style="margin-left: -6px; margin-right: -6px; width: calc(100% + 12px);">
        <tr>
            <td class="kpi-hero" style="width: 34%;">
                <div class="kpi-title" style="color: #a7f3d0;">Total Omzet Produk</div>
                <div class="kpi-value" style="color: #34d399;">{{ $rupiah($total) }}</div>
                <div class="kpi-sub" style="color: #d1fae5;">Akumulasi Penjualan Barang</div>
            </td>
            <td class="kpi-card" style="width: 33%; border-left: 3px solid #10b981;">
                <div class="kpi-title" style="color: #64748b;">Volume Transaksi</div>
                <div class="kpi-value" style="color: #1e293b;">{{ number_format($count, 0, ',', '.') }} <span style="font-size: 9px; font-weight: normal; color: #64748b;">Struk</span></div>
                <div class="kpi-sub" style="color: #64748b;">Total Nota Diterbitkan</div>
            </td>
            <td class="kpi-card" style="width: 33%; border-left: 3px solid #3b82f6;">
                ...
                <div class="kpi-sub" style="color: #64748b;">Nilai Belanja per Struk</div>
            </td>
        </tr>
    </table>

    {{-- TOP 10 PRODUK TERLARIS --}}
    <h2>1. Produk Terlaris (Top 10 Qty Terjual)</h2>
    <table class="data-table">
        <thead>
            <tr>
                <th style="width: 35px;" class="text-center">Rank</th>
                <th>Nama Produk</th>
                <th style="width: 100px;" class="text-right">Qty Terjual</th>
                <th style="width: 120px;" class="text-right">Porsi Penjualan</th>
            </tr>
        </thead>
        <tbody>
            @forelse($topProduk as $index => $p)
                @php
                    $qty = (int) $p->qty;
                    $pct = $sumTopQty > 0 ? round(($qty / $sumTopQty) * 100, 1) : 0;
                @endphp
                <tr>
                    <td class="text-center" style="font-weight: bold; color: #059669;">#{{ $index + 1 }}</td>
                    <td style="font-weight: 600;">{{ $p->nama }}</td>
                    <td class="text-right" style="font-weight: bold; color: #0f172a;">{{ number_format($qty, 0, ',', '.') }} pcs</td>
                    <td class="text-right">{{ $pct }}%</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="text-center" style="padding: 8px; color: #64748b; font-style: italic;">
    ...
    <h2>2. Rincian Riwayat Transaksi Produk</h2>
    <table class="data-table">
        <thead>
            <tr>
        ...
        <tbody>
            ...
            @forelse($rows as $idx => $r)
                @php
                    $buyer = $r->buyer?->user?->name ?? 'Non-member / Guest';
                ...
                <tr>
                    ...
                    <td class="text-center"><span class="{{ $badgeClass }}">{{ strtoupper($r->metode_pembayaran) }}</span></td>
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center" style="padding: 12px; color: #64748b; font-style: italic;">
                        Tidak ada transaksi produk yang sesuai filter.
                    </td>
                </tr>
            @endforelse
        </tbody>
        ...
        <tr>
            <td style="width: 55%; padding-right: 20px;">
                <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 6px 8px; font-size: 7.5px; color: #64748b; line-height: 1.4;">
                    ...
                    Laporan ini memuat rekapitulasi data penjualan produk retail BETA GYM.<br>
                    Data disusun berdasarkan transaksi resmi yang tercatat di sistem kasir.
                </div>
            </td>
