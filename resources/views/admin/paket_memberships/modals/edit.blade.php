...
@php
    ...
    $openEditOnLoad = $errors->any() && old('_method') === 'PUT' && (int) old('paket_id') === (int) $paket->id;
@endphp

<div x-show="openEditId === {{ $paket->id }} || {{ $openEditOnLoad ? 'true' : 'false' }}" x-cloak x-transition
    class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 bg-black/40 backdrop-blur-sm"
    @keydown.escape.window="openEditId = null" @wheel.prevent @touchmove.prevent @click.self="openEditId = null">
            ...
            <form method="POST" action="{{ route('admin.paket_memberships.update', $paket) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="deskripsi_edit_{{ $paket->id }}" name="deskripsi" rows="3"
                        class="w-full rounded-xl border bg-brand-shell text-sm text-text-main px-3 py-2 border-brand-borderSoft
                               focus:outline-none focus:ring-2 focus:ring-primary-dark focus:border-transparent
                               @error('deskripsi') border-danger ring-danger-soft @enderror">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-xs text-danger mt-1">{{ $message }}</p>
                    @enderror
                </div>
