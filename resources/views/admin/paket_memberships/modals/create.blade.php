...
<div x-show="openCreate" x-cloak x-transition
    ...
    <div @click.away="openCreate = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b-2 border-brand-borderSoft/80">
            <div>
                <h2 class="text-xl font-semibold text-text-main">Tambah Paket Membership</h2>
            ...
            <form method="POST" action="{{ route('admin.paket_memberships.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="tipe_create" name="tipe" required
                            ...
                            <option value="" disabled {{ old('tipe') === null ? 'selected' : '' }}>Pilih tipe
                                paket</option>
                ...
                <div>
                    ...
                    <textarea id="deskripsi_create" name="deskripsi" rows="3"
                        class="w-full rounded-xl border bg-brand-shell text-sm text-text-main px-3 py-2 border-brand-borderSoft focus:outline-none focus:ring-2 focus:ring-primary-dark focus:border-transparent @error('deskripsi') border-danger ring-danger-soft @enderror">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="text-xs text-danger mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-[11px] text-text-muted mt-1">
